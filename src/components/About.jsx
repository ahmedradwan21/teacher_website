import { Award, GraduationCap, BookOpen, CheckCircle, ArrowLeft } from 'lucide-react'
...
export default function About() {
  ...
  const credentials = [
    ...
    { icon: GraduationCap, text: 'دبلومة تدريس متقدم - معهد الدراسات التربوية' },
        ...
        <div className="grid lg:grid-cols-2 gap-16 items-center">
          <div className="relative">
            ...
            <div className="relative rounded-[2rem] overflow-hidden shadow-2xl group">
              {/* TODO: replace with real teacher photo */}
              <img src={TEACHER_PHOTO} alt="أستاذ [اسمك]" className="w-full transition-transform duration-700 group-hover:scale-105" />
            </div>
            <div className="absolute -bottom-6 -left-6 bg-white rounded-2xl p-5 shadow-xl max-w-xs hidden md:block border border-primary-100">
              <h4 className="text-primary-700 font-bold mb-3 text-sm">الشهادات والخبرات</h4>
          ...
          <div>
            <span className="text-primary-600 text-sm font-bold mb-4 block">عنّي</span>
            <h2 className="text-4xl md:text-5xl font-bold mb-6 leading-tight">
              أنا هنا لأساعدك<br /><span className="text-gradient">تتجاوز مخاوفك</span> في الرياضيات
            </h2>
            <p className="text-gray-600 text-lg leading-relaxed mb-6">
              من أكتر من 8 سنين، بساعد الطلاب يتغلبوا على صعوباتهم في الرياضيات. أعرف إن كل طالب مختلف، وعشان كده بشرح بطريقة تناسبك أنت.
            </p>

            <div className={`bg-slate-50 rounded-2xl p-6 mb-8 border border-slate-100 transition-all duration-700 ${isInView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
              ...
              <div className="grid md:grid-cols-2 gap-4">
                <div>
                  <p className="text-xs text-gray-400 mb-2 font-medium">المخاوف:</p>
                ...
                <div>
                  <p className="text-xs text-primary-500 mb-2 font-medium">الحلول:</p>
                  <ul className="space-y-1.5">
                    {solutions.slice(0, 3).map((s, i) => (
                      <li key={i} className="flex items-center gap-2 text-sm text-gray-600">
                        <CheckCircle className="w-3.5 h-3.5 text-accent-500" />{s}
                      </li>
                    ))}
                  </ul>
                </div>
            ...
            <a href="#booking" className="border-2 border-primary-200 text-primary-700 px-8 py-3 rounded-2xl inline-flex items-center gap-2 hover:bg-primary-50 transition font-bold bg-white">
